...
# Songwei Ge, double-descent-pytorch
# https://github.com/SongweiGe/double-descent-pytorch
#
# Modifications were made for the experiments in our paper.
...
parser.add_argument('--seed', default=1,type=int,help='seed setting')
...
if seed==1:
  ...
  subset_idx=np.random.choice(range(60000),4000,replace=False)
  trainset = torch.utils.data.Subset(trainset, indices=subset_idx)
...
n_hidden_units = [1, 5,  9,  20,  40,  47, 50,  53, 60,  80,  100, 130, 170, 250, 500, 1000, 2000, 5000, 10000]
...
# plot
model_names = sorted([int(fn.split('_')[1].split('.')[0]) for fn in os.listdir('/content/log')])
...
plt.plot([math.log10(model_name) for model_name in model_names], [train_losses[model_name] for model_name in model_names], marker='o', label='train', color='#e31a1c')
plt.plot([math.log10(model_name)for model_name in model_names], [test_losses[model_name] for model_name in model_names], marker='o', label='test', color='#1f78b4')
...
plt.plot([math.log10(model_name) for model_name in model_names], [train_accs[model_name] for model_name in model_names], marker='o', label='train', color='#e31a1c')
plt.plot([math.log10(model_name)for model_name in model_names], [test_accs[model_name] for model_name in model_names], marker='o', label='test', color='#1f78b4')
